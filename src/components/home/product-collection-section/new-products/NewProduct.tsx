  ...
  return (
    <div className="w-full h-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
      {products.map((product) => (
        <div key={product.id}>
          <div className="relative w-full h-[65%] md:h-[60%] overflow-hidden group">
            <Image
              src={product.image}
              alt="producto"
              fill
            ...
            <Image
              ...
              alt="producto"
              fill
              className="object-cover scale-200 transition-opacity hidden md:block duration-600 opacity-0 group-hover:opacity-100"
              sizes="(max-width: 768px) 100vw, 33vw"
            />
