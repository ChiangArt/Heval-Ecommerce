...
export default function CollectionGridItem({ collection }: Props) {
  const [displayImage, setDisplayImage] = useState(collection.images[0]);

  // Calculamos el precio con descuento si existe
  const discountAmount = collection.discount
    ? collection.price * collection.discount
    : 0;
  const finalPrice = (collection.price - discountAmount).toFixed(2);
    ...
    <div className="flex flex-col justify-between bg-white shadow-md rounded-2xl overflow-hidden transition lg:hover:shadow-xl h-full">
      <div className="relative w-full aspect-[3/3] overflow-hidden group">
        <Image
          ...
          onMouseEnter={() => setDisplayImage(collection.images[1])}
          onMouseLeave={() => setDisplayImage(collection.images[0])}
        />
        {collection.discount > 0 && (
          <div className="absolute top-2 right-2 bg-[#DCF3FF]  text-xs font-semibold px-2 py-1 rounded-full shadow-sm">
            -{Math.round(collection.discount * 100)}%
          </div>
        )}
